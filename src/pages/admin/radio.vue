...
// const { state, next, prev } = useCycleList(radioStations)
// const { playing, currentTime, waiting, volume, muted } = useMediaControls(audioRadioRef)
// const { results: resultsSearchStation } = useFuse(inputSearchStation, radioStations, {
//   fuseOptions: {
//     keys: ['name', 'homepage'],
//   },
...
//       const station = radioStations.value.find(i => i.url_resolved === stationUrlResolvedQuery.value)
//       if (station)
//         state.value = station
//     }
//   }
...
//   if (data.value)
//     radioStations.value = useArrayUnique(data.value.filter(i => i.favicon), (a, b) => a.name.trim() === b.name.trim()).value.map((s, index) => ({ ...s, isFavorite: false, srcHasError: false, index }))
...
//   const station = radioStations.value.find(i => i.url_resolved === src)
